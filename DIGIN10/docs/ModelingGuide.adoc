...
The DIGIN10 model includes examples for converting the GL.xml files to GeoJSON-LD.
The official GeoJSON-LD context has been used as a base for the example files. The example files include the context required for our needs (cim, nc, rdf, dcterms, dct, prov).

[red]#We use prov, dct and dcterms for a replacement for the FullModel information.#

The GeoJSON example files represent a feature collection with with features, meaning the example files contain a collecion of CIM entity locations. +
...
For properties DIGIN recommends using the @id with id type(urn:uuid) for mRID. Otherwise the only other attribute included for each CIM entity is the cim:IdentifiedObject.name.

PositionPoint is included as geometry. Each coordinate object includes xPosition, yPosition and zPosition. The coordinate system is WGS 84. The @id of the geometry features corresponds to the mRID of Location in the XML files.
...
Contrary to the XML files, we include the equipment in the GeoJSON files as properties [red]#Why allowing this now?#.
